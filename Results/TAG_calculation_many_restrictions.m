%TAG calculation adapted specifically for scenario 7 'Many restrictions'
%% Loading data
...
Resp_TSO_NS1_2(1) = 0.206; %BE 20.6%
Resp_TSO_NS2_2(1) = 0.206;
Resp_TSO_NS3_2(1) = 0.206;

Resp_TSO_NS1_2(2) = 0.311; %DE 31.1%
Resp_TSO_NS2_2(2) = 0.311;
Resp_TSO_NS3_2(2) = 0.311;

Resp_TSO_NS1_2(3) = 0.181; %DK 18.1%
Resp_TSO_NS2_2(3) = 0.181;
Resp_TSO_NS3_2(3) = 0.181;

Resp_TSO_NS1_2(5) = 0.219; %NL 21.9%
Resp_TSO_NS2_2(5) = 0.219;
Resp_TSO_NS3_2(5) = 0.219;

Resp_TSO_NS1_2(6) = 0.083; %PL 8.3%
Resp_TSO_NS2_2(6) = 0.083;
Resp_TSO_NS3_2(6) = 0.083;
%% TAG version 2
...
if OffCurt(1) > 0
    ...
    Comp_TSO_NS1_2 = transpose(Comp_MW_NS1_2*Resp_TSO_NS1_2.*max(ZP-NP(10),0)); % TSO compensation payments
end 

% NS2
if OffCurt(2) > 0 
    Comp_MW_NS2_2 = OffGA(2); % Compensated amount - total generation available
    Comp_TSO_NS2_2 = transpose(Comp_MW_NS2_2*Resp_TSO_NS2_2.*max(ZP-NP(11),0)); % TSO compensation payments
end

% NS3
if OffCurt(3) > 0 
    Comp_MW_NS3_2 = OffGA(3); % Compensated amount - total generation available
    Comp_TSO_NS3_2 = transpose(Comp_MW_NS3_2*Resp_TSO_NS3_2.*max(ZP-NP(12),0)); % TSO compensation payments
end
...
if OffCurt(1) > 0
    ...
    Comp_TSO_NS1_3 = transpose(Comp_MW_NS1_3*Resp_TSO_NS1_3.*max(ZP-NP(10),0)); % TSO compensation payments
end 

% NS2
if OffCurt(2) > 0 
    Comp_MW_NS2_3 = OffCurt(2); % Compensated amount - curtailed amount
    Comp_TSO_NS2_3 = transpose(Comp_MW_NS2_3*Resp_TSO_NS2_3.*max(ZP-NP(11),0)); % TSO compensation payments
end

% NS3
if OffCurt(3) > 0 
    Comp_MW_NS3_3 = OffCurt(3); % Compensated amount - curtailed amount
    Comp_TSO_NS3_3 = transpose(Comp_MW_NS3_3*Resp_TSO_NS3_3.*max(ZP-NP(12),0)); % TSO compensation payments
end
...
Results1_WFs = Results1(:,[1:2,6]);
Results2_WFs = Results2(:,[1:2,6]);
Results3_WFs = Results3(:,[1:2,6]);
...
CR_TSO = zeros(1,9);
CR_TSO(2) = CR_DC(4) + 0.5*CR_AC(10) + 0.5*CR_AC(11) + 0.5*CR_DC(3) + 0.5*CR_DC(2); %DE earns 50% CR on the lines to PL, DK and NL 
CR_TSO(3) = CR_DC(6);
CR_TSO(5) = CR_DC(7);
CR_TSO(1) = CR_AC(2)*0.5; %BE earns 50% CR on line to NL
CR_TSO(6) = CR_AC(10)*0.5; %PL earns 50% CR on line to DE
